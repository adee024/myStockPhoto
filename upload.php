   ...
   if (isset($_FILES['image'], $_POST['title'], $_POST['description'])) {
   	...
   	if (!empty($_FILES['image']['tmp_name']) && getimagesize($_FILES['image']['tmp_name'])) {
   		if (file_exists($image_path)) {
               ...
               echo 'document.getElementById("log").innerHTML = "Image alredy exist!";';
               echo '</script>';
   		...
   		} else if ($_FILES['image']['size'] > 2048000) {
   			echo '<script language="javascript">';
               echo 'alert("Image file size too large, please choose an image less than 2mb.")';
               echo '</script>';
